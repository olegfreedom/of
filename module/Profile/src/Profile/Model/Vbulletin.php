...
define('FORUMPATH', PUBLIC_PATH . '/vb5'); // path to your forum
define('VB_ENTRY', 1);
...
//---------------------------------------------------------------------
class Vbulletin extends \Application\Base\Model { // vB_DataManager_User

    const REGISTERED_USERGROUP = 2; // typical default for registered users
    const BANNED_USERGROUP = 19; // typical default for banned
    const NOACTIVATION_USERGROUP = 3; // typical default for user awaiting activation
    const ADMIN_USERGROUP = 6;
    const PERMANENT_COOKIE = FALSE; // false=session cookies (recommended)
    ...
    {
        // NOT require_once
        require FORUMPATH . '/core/includes/config.php';
        ...
        if(isset($params['username']) && isset($params['password'])){
            $select = $this->select()
                ...
                ->where(array(
                    ...
                    'password' => $this->expr('MD5(CONCAT(MD5(?), salt))', $params['password']),
                    $this->where()
                         ->notEqualTo('usergroupid', self::ADMIN_USERGROUP),
                ))
                ->limit(1);
    ...
    public function changePassword($user_id, $password)
    {
        ...
        $res = $this->update(self::$config['Database']['tableprefix'] . 'user')
            ->values(array(
    ...
    public function registerNewUser($userAr)
    {
        $userId = FALSE;
        ...
        {
            $salt = $this->generateSalt();
            $username = htmlspecialchars_uni($userAr ['username']);
            $currentTime = time();

            $insert = $this->insert(self::$config['Database']['tableprefix'] . 'user')
                ->values(array(
                    'username' => $username,
                    'salt' => $salt,
                    'password' => $this->generatePassword($userAr ['password'], $salt),
                    'email' => $username,
                    'joindate' => $currentTime,
                    'lastvisit' => $currentTime,
                    'lastactivity' => $currentTime,
                    'usergroupid' => self::NOACTIVATION_USERGROUP,
                    'passworddate' => $this->expr('FROM_UNIXTIME(?)', $currentTime),
                    'showvbcode' => 1,
                    'reputationlevelid' => 5,
                    'timezoneoffset' => 0,
                    'options' => 112217111,
            ...
            $userId = $this->insertId($result);

            if ( !empty($userId) )
            {
                $insert = $this->insert(self::$config['Database']['tableprefix'] . 'usertextfield')
                    ->values(array(
                        'userid' => $userId
                    ));
                $result = $this->execute($insert);

                $insert = $this->insert(self::$config['Database']['tableprefix'] . 'userfield')
                    ->values(array(
                        'userid' => $userId
                    ));
                $result = $this->execute($insert);
    ...
    {
        $set = $this->update(self::$config['Database']['tableprefix'] . 'user')
            ->set(array(
                'usergroupid' => self::REGISTERED_USERGROUP,
            ))
            ->where(array('userid' => $user_id));

        $res = $this->execute($set);

        return $res;
    }

    public function setBirthday($user_id, $birthdayDate)
    {
        $res = NULL;

        if ( !empty($birthdayDate) )
        {
            $tmpAr = explode('.', $birthdayDate);
// 28.01.1980
            $set = $this->update(self::$config['Database']['tableprefix'] . 'user')
                ->set(array(
                    // 01-28-1980
                    'birthday' => $tmpAr [1] . '-' . $tmpAr [0] . '-' . $tmpAr [2],
                    // 1980-01-28
                    'birthday_search' => $tmpAr [2] . '-' . $tmpAr [1] . '-' . $tmpAr [0],
                ))
                ->where(array('userid' => $user_id));
...
//---------------------------------------------------------------------
    private function fetch_userinfo_from_username($username, $option=0, $languageid=0)
    {
        ...
        $useridq = $this->fetchRowSelect($select);
    ...
    public function update_user(&$userdata)
    {
        ...
        if (isset($vbuser['password']))
            vbsetcookie('password',
                md5($this->vbulletin->userinfo['password'].COOKIE_SALT),
...
//---------------------------------------------------------------------
    public function login($vbuser,$acceptgroups=false)
    {
        ...
        $userinfo = $this->fetch_userinfo_from_username($vbuser['username']);
        require_once FORUMPATH . '/includes/api/interfaceabstract.php';
        require_once FORUMPATH . '/includes/api/interface/noncollapsed.php';

        $VbApi = new VB_API_CALLS();
        var_dump($VbApi->api_login($userinfo ['username'], $userinfo ['password']));
        die();
        ...
        //Verify login via VB
/*
        // require_once FORUMPATH . '/vb5/includes/api/interface/noncollapsed.php';
        $_REQUEST['api_m'] = ;

        // Client ID
        $_REQUEST['api_c'] = intval;

        // Access token
        $_REQUEST['api_s'] = ;

        // Request Signature Verification Prepare (Verified in vB_Session_Api)
        $api_sig = $_REQUEST['api_sig'] = ;
        $_REQUEST['api_v'] = 505;


        require_once FORUMPATH . '/core/api.php';
*/
        $VbApi = new VB_API_CALLS();
        // $VbApi->api_login($username, $password);


        $_POST ['do'] = 'login';


        // require_once FORUMPATH . '/core/vb/user.php';
