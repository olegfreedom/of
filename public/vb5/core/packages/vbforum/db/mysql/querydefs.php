	...
	(
		...
		'getLastData' => array(vB_dB_Query::QUERYTYPE_KEY => vB_dB_Query::QUERY_SELECT,
			...
			WHERE cl.parent = {parentid} AND cl.depth > 0
				AND n.inlist > 0 AND n.publishdate <= {timenow} AND n.showpublished > 0 AND n.showapproved > 0 AND n.contenttypeid NOT IN ({excludeTypes})
	...
	***/
	function moveNodes($params, $db, $check_only = false)
	{
		...
		if ($parent['contenttypeid'] == vB_Types::instance()->getContentTypeId('vBForum_Channel'))
		{
			...
			// some channels don't have a route, like the Main Forum
			if (!empty($newrouteid))
			{
		...
		// update show fields
		$sql = "UPDATE " . TABLE_PREFIX . "node ch
			INNER JOIN " . TABLE_PREFIX . "closure cl ON cl.child = ch.nodeid AND cl.parent = " . $params['to_parent'] . "
			INNER JOIN " . TABLE_PREFIX . "closure cl2 ON cl2.child = cl.child AND cl2.parent IN (" . implode(',',$params['nodeids']) . ")
			INNER JOIN " . TABLE_PREFIX . "node p ON cl.parent = p.nodeid
			SET ch.showopen = CASE WHEN p.open > 0 AND ch.open > 0 THEN 1 ELSE 0 END,
				...
				ch.showpublished = CASE WHEN p.unpublishdate > 0 AND p.unpublishdate < " . vB::getRequest()->getTimeNow() . " THEN 0 ELSE 1 END\n/**" .
			__FUNCTION__ . (defined('THIS_SCRIPT') ? '- ' . THIS_SCRIPT : '') . "**/";
		$result = $db->query_write($sql);

		//showopen
		$sql = "UPDATE " . TABLE_PREFIX . "node filter
				INNER JOIN " . TABLE_PREFIX . "closure cl ON filter.nodeid = cl.child
				    AND (filter.open = 0 OR filter.showopen = 0) AND cl.parent IN ( " . implode(',',$params['nodeids']) . ") AND depth > 0
				INNER JOIN " . TABLE_PREFIX . "closure cl2 ON cl2.parent = filter.nodeid
				INNER JOIN " . TABLE_PREFIX . "node target ON target.nodeid = cl2.child
		...
		$sql = "UPDATE " . TABLE_PREFIX . "node filter
				INNER JOIN " . TABLE_PREFIX . "closure cl ON filter.nodeid = cl.child
				    AND (filter.approved = 0 OR filter.showapproved = 0) AND cl.parent IN ( " . implode(',',$params['nodeids']) . ") AND depth > 0
				INNER JOIN " . TABLE_PREFIX . "closure cl2 ON cl2.parent = filter.nodeid
				INNER JOIN " . TABLE_PREFIX . "node target ON target.nodeid = cl2.child
		...
		$sql = "UPDATE " . TABLE_PREFIX . "node filter
				INNER JOIN " . TABLE_PREFIX . "closure cl ON filter.nodeid = cl.child
				    AND ((filter.unpublishdate > 0 AND filter.unpublishdate < " . vB::getRequest()->getTimeNow() . ") OR filter.showpublished = 0) AND cl.parent IN ( " . implode(',',$params['nodeids']) . ") AND depth > 0
				INNER JOIN " . TABLE_PREFIX . "closure cl2 ON cl2.parent = filter.nodeid
				INNER JOIN " . TABLE_PREFIX . "node target ON target.nodeid = cl2.child
				SET target.showpublished = 0\n/**" .
			__FUNCTION__ . (defined('THIS_SCRIPT') ? '- ' . THIS_SCRIPT : '') . "**/";
		$result = $db->query_write($sql);
	...
	public function cloneNodes($params, $db, $check_only = false)
	{
		...
		{
			$children = vB::getDbAssertor()->assertQuery('vBForum:closure',
				array(
					vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_SELECT,
			...
			foreach ($children as $k => $closure)
			{
				...
				//publishdate field can be null
				if (empty($closure['publishdate']))
				{
					$closure['publishdate'] = '0';
				}
				...
				else
				{
					...
					$moveTo = $starter;
					if (!empty($oldnewnodes[$child['parentid']]))
					{
...
// 		}
	...
	public function calculateStats($params, $db, $check_only = false)
	{
		...
		else
		{
			...
			//create a temp table.
			$db->query_write("CREATE TABLE " . TABLE_PREFIX . "tmp_nodestats AS SELECT node.parentid AS nodeid, count(nv.nodeid) AS tmpvisitors,
			...
			$sql = "INSERT INTO  " . TABLE_PREFIX . "nodestats(nodeid, visitors, replies, dateline)
				...
				FROM " . TABLE_PREFIX . "tmp_nodestats AS tmp
				ON DUPLICATE KEY UPDATE visitors = visitors + tmpvisitors, replies = replies + tmpreplies; /**vBForum:calculateStats called from cron **/";
			$db->query_write($sql);

			$sql = "INSERT INTO  " . TABLE_PREFIX . "nodestatreplies (nodeid, replies) SELECT nodeid, tmpreplies FROM " . TABLE_PREFIX . "tmp_nodestats
			...
			$db->query_write("DROP TABLE " . TABLE_PREFIX . "tmp_nodestats; /**vBForum:calculateStats called from cron **/");
