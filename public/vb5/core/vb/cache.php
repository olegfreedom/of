 ...
 *
	...
	public function write($key, $data,  $lifetime_mins= false, $events = false)
	{
		...
		if (self::$cacheLogging)
		{
			self::logCacheAction($key, self::CACHE_LOG_WRITE, $this->cachetype, strlen(serialize($cache)));
		}
		return $key;
	}
	...
	****/
	...
	public function readSingle($key, &$write_lock = false, $save_meta = false)
	{
		...
		if (isset($this->values_read[$key]))
		{
			...
			if (self::$cacheLogging)
			{
				self::logCacheAction($key, self::CACHE_LOG_HASVALUE, $this->cachetype);
			}
		...
		if (self::$cacheLogging)
		{
			...
			{
				self::logCacheAction($key, self::CACHE_LOG_READSUCCESS, $this->cachetype);
			}
		}
		return $cache['data'];
	}
...
/*Clean=========================================================================*/
	...
	protected static function logCacheAction($cacheid, $action, $type = 4, $size = 0)
	{
		...
		{
			self::$actions[$type][$cacheid] = array('cacheid' => $cacheid,
			'randomkey' => $random, 'type' => $type,
			'writes' =>0 , 'misses' => 0, 'hits' => 0 ,
			'rereads' => 0,	'remiss' => 0, 'clears' => 0,
			'time' => vB::getRequest()->getTimeNow(), 'size' => 0);
		}
		switch ($action)
		{
	...
	**/
	protected function logCacheActivity()
	{
		...
		foreach (self::$actions as $type => $actions)
		{
			if (!empty($actions))
			{
				$assertor->assertQuery('vBInstall:cachelog',
				array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_MULTIPLEINSERT,
				vB_dB_Query::FIELDS_KEY => array('cacheid', 'randomkey', 'cachetype',
				'writes', 'misses', 'hits',
				'rereads',	'remiss', 'clears', 'time', 'size'),
				vB_dB_Query::VALUES_KEY => $actions));
			}
		}
		self::$actions = array(self::CACHE_STD => array(),
			self::CACHE_FAST => array(),
			self::CACHE_LARGE => array(), 4 => array());;
	}
