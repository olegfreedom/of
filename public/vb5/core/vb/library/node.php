...
class vB_Library_Node extends vB_Library
{
	...
	**/
	...
	{
		$node = $withJoinableContent? $this->getNodeFullContent($nodeid) : $this->getNodeBare($nodeid);
	...
	**/
	...
	public function addFullContentInfo($nodeList, $options = array())
	{
		...
		if (!empty($nodeIds))
		{
			$attachCountQry = vB::getDbAssertor()->getRows('vBForum:getDescendantAttachCount', array('nodeid' => $nodeIds));
			foreach ($attachCountQry as $count)
			{
				$attachCounts[$count['nodeid']] = $count['count'];
			}

			$photoCountQry = vB::getDbAssertor()->getRows('vBForum:getDescendantPhotoCount', array('nodeid' => $nodeIds,
	...
	***/
	...
	function setApproved($approveNodeIds, $approved = true)
	{
		...
		foreach ($nodeIds AS $node)
		{
			...
			$searchAPI->attributeChanged($thisNode['nodeid']);
		}
