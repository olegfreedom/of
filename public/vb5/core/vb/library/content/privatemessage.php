	...
	**/
	...
	public function listNotifications($data, $userid)
	{
		...
		foreach ($notificationQry AS $notifyRec)
		{
			...
			if ($notifyRec['about'] == 'comment')
			{
				$notifyRec['lastcontentavatar'] = $userApi->fetchAvatar($notifyRec['userid']);
			}
		...
		if (!empty($discussions))
		{
			...
			foreach ($notifications AS $key => &$notification)
			{
				...
				if (empty($notification['lastcontentavatar']))
				{
	...
	public function acceptRequest($nodeid)
	{
		...
		*/
		...
		if ($result)
		{
			...
			return vB_Phrase::fetchSinglePhrase($resultphrase);
		}
		else
		{
			return false;
