...
class vB_Library_Content_Channel extends vB_Library_Content
{
	...
	public function add($data, array $options = array('nodeonly' => false))
	{
		...
		if (isset($data['filedataid']) AND
				vB::getUserContext()->getChannelPermission('forumpermissions', 'canuploadchannelicon', $nodeid))
		{
			vB::getDbAssertor()->assertQuery('incrementFiledataRefcount', array('filedataid' => $data['filedataid']));
		}
		return $nodeid;
	}
