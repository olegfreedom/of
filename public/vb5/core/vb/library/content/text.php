...
class vB_Library_Content_Text extends vB_Library_Content
{
	...
	***/
	...
	protected function sendEmailNotification($data)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					case 'reply':
						$mailPhrase = 'reply_' . (($currentNode['starter'] == $currentNode['parentid']) ? 'thread' : 'post');
						break;
					case 'comment':
						$mailPhrase = 'comment_' . (($currentNode['starter'] == $currentNode['parentid']) ? 'thread' : 'post');
						break;
					case 'threadcomment':
	...
	/*** Processing after a move. In this case, update the starter field and set the text/total counts and "last" data.
	*
	...
	*/
	...
	{
		parent::afterMove($nodeid, $oldparent, $newparent, $oldnode);
		$newParentNode = $this->nodeLibrary->getNodeBare($newparent);
