...
class vB_Library_Style extends vB_Library
{
	...
	protected function addStylevars(&$styles, $styleids = false)
	{
		...
		if (!empty($styleids))
		{
			...
			$notFound = $this->preCacheStylevars($styleids);
	...
	*
	*	@return	mixed	array of integers, which may be empty- those styles that must be queried.
	*/
	protected function preCacheStylevars($styleids)
	{
		$notFound = array();
		foreach($styleids AS $key => $styleid)
		{
			...
			{
				$this->stylecache[$styleid]['newstylevars'] = $this->stylevarcache[$styleid];
				unset($styleids[$key]);
			}
			else
			{
		...
		foreach ($notFound as $styleid => $cacheKey)
		{
			if (!empty($found[$cacheKey]))
			{
				...
				$this->stylecache[$styleid]['newstylevars'] = $found[$cacheKey];
	...
	protected function setStylevars($styleid, $data)
	{
		$styleid = intval($styleid);
		$this->stylevarcache[$styleid] = $data;
		$this->stylecache[$styleid]['newstylevars'] = $data;
	...
	public function fetchStyleByID($styleid, $abort = true)
	{
		...
		if(isset($this->stylecache[$styleid]))
		{
			...
			if (!empty($this->stylecache[$styleid]['parentid']))
			{
				$parents = explode(',', $this->stylecache[$styleid]['parentid']);
			...
			$this->addStylevars($this->stylecache, $parents);
