...
class vB_Api_Node extends vB_Api
{
	...
	public function moveNodes($nodeids, $to_parent, $makeTopic = false, $newtitle = false, $modlog = true, array $leaveRedirectData = array())
	{
		...
		foreach ($nodes as $node)
		{
				...
				(($currentUserid == $node['userid']) AND $userContext->getChannelPermission('forumpermissions', 'canmove', $node['nodeid'], false, $node['parentid'])))
			{
				...
				$this->contentLibraries[$node['nodeid']] = vB_Library::instance('Content_' . vB_Types::instance()->getContentTypeClass($node['contenttypeid']));
		...
		foreach ($movedNodes as $nodeid)
		{
			...
			$this->contentLibraries[$nodeid]->afterMove($nodeid, $oldparents[$nodeid], $newparents[$nodeid], $oldnodeInfo[$nodeid]);
		}
	...
	*/
	...
	public function incrementNodeview($nodeid)
	{
		...
		// we're not implementing the threadviewslive option, VBV-5055
