	...
	protected function loadExistingSession($sessionhash, $userid, $password)
	{
		...
		if ($this->fetchStoredSession($sessionhash))
		{
			...
			if (!empty($this->vars['userid']))
			{
				...
				if ($cached !== false)
				{
					if (!empty($cached['lastactivity']) AND ($cached['lastactivity'] >  vB::getRequest()->getTimeNow() - $this->cookietimeout) AND
						...
						($this->fetch_substr_ip($cached['vars']['host']) == $this->fetch_substr_ip($cached['vars']['host'])))
					{
						...
						//during login, the session can stay the same but the user info change
						if (!empty($this->data['channelPerms']) OR !empty($this->data['channelPerms']['userid']) AND ($this->vars['userid'] != $this->data['channelPerms']['userid']))
						{
		...
		//For guests, get the cached channelPerms data.
		if (empty($this->vars['userid']))
		{
			$channelPerms = vB_Cache::instance(vB_Cache::CACHE_STD)->read('vB_ChannelPerms_Guest');

			if ($channelPerms AND is_array($channelPerms))
			{
				$this->data['channelPerms'] = $channelPerms;
			}
		}
	...
	public function save()
	{
		...
		if (!$this->skipcache)
		{
			...
			if ($this->vars['userid'] > 0)
			{
				...
				//we don't need to save any phrasegroups. But by default phrasegroup_global is set if the user has selected a custom language.
				unset($this->data['userinfo']['phrasegroup_global']);
			...
			else if ($this->cacheChanged)
			{
				//we only save the channelPerms for guests
				vB_Cache::instance(vB_Cache::CACHE_STD)->write('vB_ChannelPerms_Guest', $this->data['channelPerms'], 1440,
					array('perms_changed', 'userChg_0', 'userPerms_0'));
	...
	*/
	...
	public function loadLanguage()
	{
		...
		if(is_array($allLanguages) AND !array_key_exists($this->vars['languageid'], $allLanguages)){
			if (!empty($this->userinfo['languageid']) AND array_key_exists($this->vars['languageid'], $allLanguages))
			{
