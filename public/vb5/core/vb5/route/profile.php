...
class vB5_Route_Profile extends vB5_Route
{
	...
	public function __construct($routeInfo, $matches, $queryString = '', $anchor = '')
	{
		...
		if (!$currentUser->hasPermission('genericpermissions', 'canviewmembers'))
		{
			throw new vB_Exception_NodePermission('profile');
		}
	...
	*/
	...
	public function getUrl()
	{
		...
		// check if user can see infractions tab, used below
		$showInfractionsTab = vB_Api::instanceInternal('content_infraction')->canViewUserInfractions($this->arguments['userid']);
		// add it as an argument. This eventually becomes a page argument, accessible from the widget_profile template
		$this->arguments['showInfractionsTab'] = $showInfractionsTab;
		
		// append the tab to URL only if it's a valid tab. 
		if (isset($this->arguments['tab']) AND in_array($this->arguments['tab'], self::$availableTabs))
		{
			// but not the infractions tab if they're not allowed to see it.
			if ( !($this->arguments['tab'] == 'infractions' AND !$showInfractionsTab) )
			{
		...
		// robots, do not index member/.../infractions
		if (isset($this->arguments['tab']) AND in_array($this->arguments['tab'], self::$doNotIndexTabs))
		{
			$this->arguments['noindex'] = true;
		}
		
		// append the page number if pagenum argument is set & if a tab with pagination is set
		if ( isset($this->arguments['tab']) AND in_array($this->arguments['tab'], self::$tabsWithPagination) AND
			isset($this->arguments['pagenum']) AND is_numeric($this->arguments['pagenum']) AND $this->arguments['pagenum'] > 1
		)
		{
