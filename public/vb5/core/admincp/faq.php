...
define('CVS_REVISION', '$RCSfile$ - $Revision: 75338 $');
...
if ($_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'add')
{
	...
	foreach($languages AS $languageid => $lang)
	{
		...
		print_input_row("{$vbphrase['title']} <dfn>(" . construct_phrase($vbphrase['x_translation'], "<b>{$lang['title']}</b>") . ")</dfn>", "title[{$lang['languageid']}]", $faqphrase["{$lang['languageid']}"]['title'], 1, 69, 0, $lang['direction']);
		// reset bgcounter so that both entries are the same colour
		$bgcounter --;
		print_textarea_row("$vbphrase[text_gcpglobal] <dfn>(" . construct_phrase($vbphrase['x_translation'], "<b>{$lang['title']}</b>") . ")</dfn>", "text[{$lang['languageid']}]", $faqphrase["{$lang['languageid']}"]['text'], 4, 70, 1, 1, $lang['direction']);
