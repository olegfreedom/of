...
define('CVS_REVISION', '$RCSfile$ - $Revision: 77175 $');
...
if ($_REQUEST['do'] == 'findupdates')
{
	...
	foreach($languages AS $language)
	{
		if (is_array($customcache["{$language['languageid']}"]))
		{
			print_description_row($language['title'], 0, 2, 'thead');
			foreach($customcache["{$language['languageid']}"] AS $phraseid => $phrase)
			{
...
if ($_REQUEST['do'] == 'add')
{
	...
	foreach($languages AS $lang)
	{
		$resizer = "<div class=\"smallfont\"><a href=\"#\" onclick=\"return resize_textarea(1, 'text_{$lang['languageid']}')\">$vbphrase[increase_size]</a> <a href=\"#\" onclick=\"return resize_textarea(-1, 'text_$_languageid')\">$vbphrase[decrease_size]</a></div>";

		print_label_row(
			construct_phrase($vbphrase['x_translation'], "<b>{$lang['title']}</b>") . " <dfn>($vbphrase[optional])</dfn><br /><input type=\"button\" class=\"button\" value=\"$vbphrase[copy_default_text]\" tabindex=\"1\" onclick=\"copy_default_text({$lang['languageid']});\" />" . $resizer,
			"<textarea name=\"text[{$lang['languageid']}]\" id=\"text_{$lang['languageid']}\" rows=\"5\" cols=\"60\" tabindex=\"1\" wrap=\"virtual\" dir=\"{$lang['direction']}\"></textarea>"
		);
...
if ($_REQUEST['do'] == 'edit')
{
	...
	else
	{
		...
		foreach($languages AS $lang)
		{
			$resizer = "<div class=\"smallfont\"><a href=\"#\" onclick=\"return resize_textarea(1, 'text_{$lang['languageid']}')\">$vbphrase[increase_size]</a> <a href=\"#\" onclick=\"return resize_textarea(-1, 'text_{$lang['languageid']}')\">$vbphrase[decrease_size]</a></div>";

			print_label_row(
				construct_phrase($vbphrase['x_translation'], "<b>{$lang['title']}</b>") . " <dfn>($vbphrase[optional])</dfn><br /><input type=\"button\" class=\"button\" value=\"$vbphrase[copy_default_text]\" tabindex=\"1\" onclick=\"copy_default_text({$lang['languageid']});\" />" . $resizer,
				"<textarea name=\"text[{$lang['languageid']}]\" id=\"text_{$lang['languageid']}\" rows=\"5\" cols=\"60\" tabindex=\"1\" wrap=\"virtual\" dir=\"{$lang['direction']}\">" . htmlspecialchars_uni($text["{$lang['languageid']}"]) . "</textarea>"
			);
