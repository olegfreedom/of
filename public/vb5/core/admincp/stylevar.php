...
define('CVS_REVISION', '$RCSfile$ - $Revision: 77046 $');
define('NOZIP', true);
		...
		foreach ($updated_stylevars AS $stylevarid)
		{
			...
			foreach ($update_properties as $property)
			{
				...
				if ($stylevar_vals[$property] != $stylevar_vals[$orig_prop_name])
				{
					...
					AND
					($stylevar_vals[$stylevar_prop_name] != $original_values[$stylevarid][$stylevar_prop_name])
				)
				{
					// Otherwise set the stylevar equivalent and remove the prop so the rest of the app will
					// show the value of the stylevar equivalent. Like stylevar_color's color
