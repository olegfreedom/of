...
define('CVS_REVISION', '$RCSfile$ - $Revision: 76019 $');
...
if ($_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'add')
{
	...
	// Create Url for the user's sent folder
	$currentUser = vB::getCurrentSession()->get('userid');
	vB_Library::instance('content_privatemessage')->checkFolders($currentUser);
	$msgFolders = vB_Library::instance('content_privatemessage')->fetchFolders($currentUser);
	$sentFolderUrl = vB5_Route::buildUrl('privatemessage|fullurl|nosession', array(
		'action' => 'sentitems',
		'folderid' => $msgFolders['systemfolders']['sent_items'],
		'pagenum' => 1,
	));
	print_yes_no_row(construct_phrase($vbphrase['save_pm_copy_default'], $sentFolderUrl), 'options[pmdefaultsavecopy]', $user['pmdefaultsavecopy']);
