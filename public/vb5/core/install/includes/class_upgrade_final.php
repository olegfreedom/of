...
class vB_Upgrade_final extends vB_Upgrade_Version
{
	...
	*/
	...
	/**
	* Step #5 - Import pagetemplates XML
	*
	*/
	function step_5()
	{
	...
	function step_8()
	{
		...
		while ($channel = array_shift($queue))
		{
			...
			$phraseLib->save('pagemeta',
				...
				array(
					'text' => array($channel['description']),
					'ismaster' => 1,
					'product' => 'vbulletin',
					't' => 0,
			...
			INSERT INTO `" . TABLE_PREFIX . "page`
				...
				VALUES
				($parentid, $pagetemplateid, '" . $this->db->escape_string($channel['title']) . "', '" . vB_Page::TYPE_CUSTOM .  "', '" . $guid . "');
				"
			);
			...
			if ($channel['category'] == 0)
			{
				...
				REPLACE INTO `" . TABLE_PREFIX . "page`
					(`parentid`, `pagetemplateid`, `title`, `pagetype`, `guid`)
					VALUES
					($parentid, $pagetemplateid, '" . $this->db->escape_string($channel['title']) . "', '" . vB_Page::TYPE_DEFAULT .  "', '" . $guid . "');
					"
				);
				$pageid = $this->db->insert_id();
