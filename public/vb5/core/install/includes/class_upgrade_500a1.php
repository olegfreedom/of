	...
	function step_23()
	{
		...
		if (empty($existingRecords) OR empty($existingRecords['pageid']))
		{
			$this->run_query(
				...
				(pageid, parentid, pagetemplateid, title, metadescription, routeid, moderatorid, displayorder, pagetype)
				VALUES
				(1, 0, 1, 'Forums', 'vBulletin Forums', 9, 0, 1, '" . vB_Page::TYPE_CUSTOM . "'),
				(2, 0, 4, 'Search', 'vBulletin Search', 24, 0, 1, 'default'),
	...
	*/
	...
	function step_158()
	{
		...
		if (isset($this->registry->products['vbcms']) AND $this->registry->products['vbcms'])
		{
			...
			SELECT $sectionTypeId AS oldcontenttypeid, node.nodeid AS oldid, ni.title,ni.description, ni.html_title, n.url,
	...
	**/
	function step_159()
	{
		if (isset($this->registry->products['vbcms']) AND $this->registry->products['vbcms'])
		{
			$process = 50;
			$sectionTypeId = vB_Types::instance()->getContentTypeID('vBCms_Section');
			$query = "SELECT node.nodeid AS parentid, ni.title,ni.description, ni.html_title, n.url,
