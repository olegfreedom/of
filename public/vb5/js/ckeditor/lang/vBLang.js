...
(function fetchCKEditorPhrases(){
	function appendName(name, obj){
		...
		if (typeof obj === 'object')
		{
			for(var item in obj)
			{
				...
				{
					$.each(appendName(item, obj[item]), function(index, value){
						namesArray.push(name + '.' + value);
					});
				}
				else
				{
					namesArray.push(name + '.' + item);
				}
			}
		}
		else
		{
			namesArray.push(name + '.' + obj[name]);
		}
	...
	for(var item in CKEDITOR.lang['vBLang'])
	{
		var appended = [];
		if (typeof CKEDITOR.lang['vBLang'][item] == 'object')
		{
			$.merge(ckeditorPhrases, appendName(item, CKEDITOR.lang['vBLang'][item]));
		}
		else
		{
			ckeditorPhrases.push(item);
		}
	}

	$.ajax({
		url:vBulletin.getAjaxBaseurl() + '/ajax/api/phrase/fetch',
		async: false,
		data:{'phrases': ckeditorPhrases},
		type:'POST',
		dataType:'json',
		success:function(result)
		{
			if (result)
			{
				var phrasesObject = parsePhrases(result);
				CKEDITOR.lang['vBLang'] = $.extend(true, CKEDITOR.lang['vBLang'], phrasesObject);
			}
			else
			{
				console.warn('Unexpected result when fetching phrases');
			}
		},
		error:function(xhr, statusText, error)
		{
			console.warn('Error when fetching phrases: {0}'.format(error));
		}
	});
	...
	function parsePhrases(phrases)
    {
		...
		function setPhraseProperty(obj,path,value){
		...
		{
			setPhraseProperty(newObj, item, phrases[item]);
		}
