...
class vB5_String
{
	...
	public static function fetchCensoredText($text)
	{
		...
		$text = self::stripBlankAscii($text, $options['censorchar'], $options['blankasciistrip']);
	 ...
	 */
	public static function fetchTrimmedTitle($title, $chars = -1, $append = true)
	{
		if ($chars == -1)
		{
			$options = vB5_Template_Options::instance()->getOptions();
			$chars = $options['options']['lastthreadchars'];
		}
